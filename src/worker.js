...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/newroom") {
      return json({ room: await createRoom(env) }, 201);
    }

    const roomMatch = url.pathname.match(/^\/api\/(?:room|ws)\/([^/]+)$/);
    if (roomMatch) {
      const room = roomMatch[1];
      const id = await checkRoom(room, env);
...
async function createRoom(env) {
  ...
  const digest = await sign(id.toString(), getRoomKey(env));

  return `${id}-${digest}`;
}

async function checkRoom(room, env) {
  if (!ROOM_RE.test(room)) {
    return null;
  }

  const [idString, hash] = room.split("-");
  const expected = await sign(idString, getRoomKey(env));
...
function getRoomKey(env) {
  return env.DOOM_KEY || DEFAULT_DEV_KEY;
}
