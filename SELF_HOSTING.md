...
After this repository is public on GitHub or GitLab, put this button in the README and replace the URL with the public repository URL:

```md
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/YOUR_NAME/YOUR_REPO)
```

Cloudflare will clone the repository into the user's Git account, provision the Durable Object binding from `wrangler.jsonc`, run the deploy command from `package.json`, and publish the Worker on that user's account.
...
The repository includes `.github/workflows/deploy.yml`. A self-hosting user can fork the repo, add these repository secrets, and run the workflow manually:
