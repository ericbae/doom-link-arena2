...
1. `doom.ossy.dev`: the installer website.
...
1. User visits `https://doom.ossy.dev`.
2. User clicks `Deploy to my Cloudflare`.
3. The installer sends them to Cloudflare's deploy URL:

   ```text
   https://deploy.workers.cloudflare.com/?url=https://github.com/ossy-dev/doom-worker
   ```
...
This keeps user credentials out of `doom.ossy.dev`.

## Fully Branded Flow

If `doom.ossy.dev` must authenticate the user and return the deployed game URL inside our own UI, then we need a backend deploy service.
...
`doom.ossy.dev` should be a small static/Worker app:
